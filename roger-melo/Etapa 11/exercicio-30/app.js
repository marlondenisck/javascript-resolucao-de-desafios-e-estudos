...
const getOperationMessage = (number1, operator, number2, calc) => 
`Resultado da operação: ${number1} ${operator} ${number2} = ${calc}.` 
...
brasil.unshift('Acre', 'Rondonia', 'Amazonia')
brasil.shift()
...
const nordeste = ['Bahia', 'Ceará', 'Pernambuco', 'Maranhão', 'Alagoas', 'Rio Grande do Norte', 'Piauí', 'Paraíba', 'Pernambuco', 'Sergipe']
...
const newBrasil = brasil.map((estado, id) => ({ id, estado }))

console.log(newBrasil)



const sevenLettersInStates = brasil.every(state => state.length > 7) 

const hasMoreThan7Letters = "Sim, todos os estados tem mais de 7 letras."
const not7LettersMessage = "Nem todos os estados tem mais de 7 letras."
const displayMessage = sevenLettersInStates ? hasMoreThan7Letters : not7LettersMessage
...
const isCearaInclude = brasil.includes('Ceará')

const isCearaMessageIncluded = 'Ceará está incluído.'
const isNotCearaMessage = "Ceará não foi incluído =/" 
const cearaMessage = isCearaInclude ? isCearaMessageIncluded : isNotCearaMessage

console.log(cearaMessage)

const newBrasilMap = newBrasil.map(({id, estado}) => ({ 
  id: id + 1,
...
const isEvenIdState = newBrasilMap.filter(state => state.id % 2 === 0)
console.log(isEvenIdState)