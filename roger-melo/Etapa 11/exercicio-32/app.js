...
const generateGifImg = (url, title) => {
...
const fetchGif = async inputValue => {
  try {
    const url = getGiphyApiUrl(inputValue)
...
const insertGifIntoDom = async inputValue => {
  const datagif = await fetchGif(inputValue)
  
  if(datagif) {
    const imageUrlGif = datagif.data[0].images.downsized.url
    const imgTitleGif = datagif.data[0].title
  
    const img = generateGifImg(imageUrlGif, imgTitleGif)
...
const handleSubmit = event => {
  ...
  const inputValue = event.target.search.value // searchInput.value
