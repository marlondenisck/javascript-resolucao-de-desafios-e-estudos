...
const getOddNumbers = randomNumbers.filter(item => item % 2 === 1)
...
const numbersLessThan501 = crazyNumbers.reduce((acc, item) => {
  if(item < 501) {
    acc += 1
  }
  return acc
...
const newMovies = tarantinoMovies.filter(item => item.release < 2000)
...
const newTvShows = tvShows.map(item => item.name )
