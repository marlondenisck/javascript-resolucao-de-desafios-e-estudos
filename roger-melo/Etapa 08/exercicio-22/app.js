...
// console.log(namesAlphabeticalOrder)
...
const characterOrderedById = characters
  //Outra forma .map(({item}) => ({ id: item.id, name: item.name}))
  //Outra forma refactorada .map(({id, name}) => ({ id, name}))
  .slice()
  .sort((item1, item2) => item1.id -item2.id)

// console.log(characters, characterOrderedById)
...
const numbersOrder = numbers.map(item => item).sort((n1, n2) => n1 - n2)
...
// const numberGreaterThan50 = some(randomNumbers, item => item > 50)
...
const peopleNamesInverted = people
  .map(people => people)
  .sort()
  .reverse()

// console.log(peopleNamesInverted)
...
const cookedIngredients = ingredients.reduce((acc, item, index, array) => {
  // Outra forma const correctWordGender = item[item.length -1] === 'a' ? 'cozida' : 'cozido'
...
const dogs = pets
  ...
  .map(({ name, age, gender, type }) => ({ 
    name, age, humanAge: age * 7, gender, type
  }))
...
const insertMovieIntoDom = title => {
  const li = document.createElement('li')
  li.textContent = title
  ul.appendChild(li)
}

const mapMoviesNames = topBrazilMovies.map(({ title }) => {
  insertMovieIntoDom(title)
})

/* ##### Forma 3 

 const movieNames = topBrazilMovies
  .reduce((acc, { title }) => acc + `<li>${{ title }.title}</li>`, '')
