...
const peoppleOrderedByScore = people
  .map(({ firstName, lastName, score }) => ({
      firstName, lastName, score
   }))
  .sort((a, b) => a.score - b.score)

//  console.log(peoppleOrderedByScore)
...
const animals = ['cão', 'gato', 'boi', 'leão', 'gnu', 'alce', 'ema', 'javali']
...
const lengthAnimalsName = animals.map(animal => animal.length)

// console.log(lengthAnimalsName)
...
const filteredCountriesAndSum = data
  .filter(({ country }) => country !== 'China')
  .reduce((acc, { population }) => acc += population, 0)

console.log(filteredCountriesAndSum)